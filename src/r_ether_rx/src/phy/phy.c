...
#define PHY_AN_LINK_PARTNER_SELECTOR    (1 << 0)
    ...
    #error  "ERROR - ETHER_CFG_PHY_MII_WAIT - Out-of-range value is defined in r_ether_rx_config.h."
#endif
...
***********************************************************************************************************************/
int16_t phy_set_autonegotiate(uint32_t ether_channel, uint16_t *pline_speed_duplex,
                               uint16_t *plocal_pause, uint16_t *ppartner_pause)
{
    uint16_t    reg;
    ...
    if (local_advertise[ether_channel] & PHY_AN_ADVERTISEMENT_PAUSE)
    {
        (*plocal_pause) |= (1 << 1);
    }
    
    if (local_advertise[ether_channel] & PHY_AN_ADVERTISEMENT_ASM_DIR)
    {
        (*plocal_pause) |= 1;
    }

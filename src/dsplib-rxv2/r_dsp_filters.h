...
typedef struct 
{ 
    uint32_t taps_n;    // number of filter taps 
	uint32_t taps_m;    // number of filter taps 
    void *   coefs;		// pointer to filter coefficients 
	void *   state;     // pointer to filter’s internal state (delay line) 
    scale_t  scale;		// scaling parameter
	uint32_t qint;		// number of integer bits in Q format of filter coefficients
    uint32_t options;	// options that specify rounding, saturation, or other behaviors 
} r_dsp_iirfilter_t; 
...
/*-----------------------*/
/* Real input: f32; Real output: f32 */
int32_t R_DSP_IIR_StateSize_f32f32 (
    const r_dsp_iirfilter_t * handle
);

r_dsp_status_t R_DSP_IIR_Init_f32f32 (
    const r_dsp_iirfilter_t * handle
);

r_dsp_status_t R_DSP_IIR_f32f32 (
    const r_dsp_iirfilter_t * handle, 
    const vector_t * input, 
    vector_t * output
);
...
/* Real input: i32; Real output: i32 */
int32_t R_DSP_IIR_StateSize_i32i32 (
    const r_dsp_iirfilter_t * handle
);

r_dsp_status_t R_DSP_IIR_Init_i32i32 (
    const r_dsp_iirfilter_t * handle
);

r_dsp_status_t R_DSP_IIR_i32i32 (
    const r_dsp_iirfilter_t * handle, 
    const vector_t * input, 
    vector_t * output
);
